...
Top30_Overall<- YrSps %>%
  ...
  slice_max(-CatchRank, n=30, with_ties = TRUE)

Top30Sp<-factor(c(Top30_Overall$NMFS.Name)) # Names of top 30 species overall by money made

# Top30Sp<-factor(c("COD, ATLANTIC", "LOBSTER, AMERICAN", "HERRING, ATLANTIC","REDFISH, ACADIAN", "HADDOCK"))
# Top30Sp<-factor(c("LOBSTER, AMERICAN", "HERRING, ATLANTIC","REDFISH, ACADIAN", "HADDOCK"))
...
# SCALING_F<-sd(Top30$OrigValue)
SCALING_F<-max(Top30$OrigValue)
...
ggplot(P_Selection%>%
         mutate(NMFS.Namev2=recode(NMFS.Namev2, "SKATE"='SKATES, RAJIDAE (FAMILY) **')),
       aes(fill=reorder(NMFS.Name, +OrigValue), y=(OrigValue)/OM, x=IYear)) + 
  geom_bar(position='stack', stat='identity') +
  # scale_fill_manual(values = col_vector) +
  scale_fill_manual(values = c("ALEWIFE"="#7FC97F",
                               "CLAM, QUAHOG, OCEAN"="#BEAED4",
  ...
  theme(legend.key.size = unit(3, 'mm'),
        ...
        legend.title = element_blank(),
        axis.text.x = element_text(angle = 0, vjust = 0.7, size=20),) +    
  labs(x="Year", y=expression('Revenue (Hundred Million $)')) + #sort out scaling if you want to use this
  # y=expression('Landings (Metric Tons x10'^12*')')
  guides(fill=guide_legend(ncol=4))
# ggsave(path = "Manuscript Plots", filename = "Revenue Barchart.png", width = 15, height = 10, dpi = 600, bg='transparent')
...
# png(width = 800, height = 800, file="Manuscript Plots/Fig 4.png", type = "cairo")
...
ggplot(data) +
  geom_point(aes(x=IYear, y=(SumVAL*SCALING_F)/OM)) + 
...
ggplot(data) +
  geom_line(aes(x=IYear, y=OrigValue/OM, colour=NMFS.Namev2), linewidth=1.5) + 
...
# ggsave(path = "Manuscript Plots", filename = "Revenue Linegraph.png", width = 15, height = 10, dpi = 600, bg='transparent')

ggplot(data) +
  geom_line(aes(x=IYear, y=log(OrigValue/OM), colour=NMFS.Namev2), linewidth=2) + 
  scale_color_manual(values = c("ALEWIFE"="#7FC97F",
                                "CLAM, QUAHOG, OCEAN"="#BEAED4",
                                "CLAM, SOFT"="#FDC086",
                                "CLAM, SURF, ATLANTIC"="#7570B3",
...
ggplot(FrontierResults, aes(x=Iteration, fill=Type)) +
  ...
  setwd("~/Portfolio project/Methods paper/Methods Paper Analysis")
# ggsave(path = "Manuscript Plots", filename = "Targets plot.png", width = 15, height = 10, dpi = 600)
...
ggplot() +
 ...
 setwd("~/Portfolio project/Methods paper/Methods Paper Analysis")
  # ggsave(path = "Manuscript Plots", filename = "Jin no decay s1 margin neg4.png", width = 15, height = 10, dpi = 600)
...
# ggsave(path = "Manuscript Plots", filename = "Riskgap numerator plot.png", width = 15, height = 10, dpi = 600)
...
# ggsave(path = "Manuscript Plots", filename = "Riskgap per dollar plot.png", width = 15, height = 10, dpi = 600)

library(ggpubr)
ggarrange(Num, Doll, common.legend=TRUE, legend="right", nrow=2,
            labels = c("a)", "b)"))
#ggsave(path = "Manuscript Plots", filename = "Riskgap plots.png", width = 15, height = 10, dpi = 600)
...
#   geom_col(aes(x=Iteration, y=TImpWeight)) +
#   facet_wrap(~NMFS.Namev2) +
#   labs(y ="Implicit Revenue Weight", x = "Year") +
...
ggplot() +
  ...
  theme(legend.title = element_text(size=14))
setwd("~/Portfolio project/Methods paper/Methods Paper Analysis")
# ggsave(path = "Manuscript Plots", filename = "Static plot.png", width = 15, height = 10, dpi = 600)
